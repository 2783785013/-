...
from until.text_until import MD5_
...
class down_video(object):
    ...
    @staticmethod
    async def __down_video(bar, queue, file_path):
        """
        :param bar: 进度条
        :param queue: 队列
        :return:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if content_length:
                ...
                with tqdm(total=content_length, unit='', desc=f'下载：{name}', unit_divisor=1024, ascii=True, unit_scale=True) as bar:
                    # 开始协程
                    return await asyncio.wait([self.__down_video(bar, queue, file_path) for i in range(count)])
            else:
                return False
    ...
    async def download_much(self, url, path, down_type=1, count=2):
        dw_list = []
        if type(url) == list:
            urls = url
        else:
        ...
        if down_type == 1:
            ...
            suffix = '.mp4'
        results = await asyncio.gather(*[self.main(url, path+MD5_(url)+suffix, count)for url in urls])
